...
class _HindiTopRatedSeriesState extends State<HindiTopRatedSeries> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('topRatedSeriesBox')) {
          topRatedSeriesBox = Hive.box('topRatedSeriesBox');
        } else {
          topRatedSeriesBox = await Hive.openBox('topRatedSeriesBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = topRatedSeriesBox.get('hindiTopRatedSeries');
      final cachedTimestamp = topRatedSeriesBox.get('hindiTopRatedSeriesTimestamp');
      ...
      await topRatedSeriesBox.put('hindiTopRatedSeries', widget.hindiTopRatedSeries);
      await topRatedSeriesBox.put('hindiTopRatedSeriesTimestamp', DateTime.now().toIso8601String());

      return widget.hindiTopRatedSeries;
    } catch (e) {
      return [];
    }
  }
