...
class HindiTvShows extends StatefulWidget {
  const HindiTvShows({super.key, required this.hindiTvShows});
  final List hindiTvShows;
...
class _HindiTvShowsState extends State<HindiTvShows> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('tvShowsBox')) {
          tvShowsBox = Hive.box('tvShowsBox');
        } else {
          tvShowsBox = await Hive.openBox('tvShowsBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = tvShowsBox.get('hindiTvShows');
      final cachedTimestamp = tvShowsBox.get('hindiTvShowsTimestamp');
      ...
      await tvShowsBox.put('hindiTvShows', widget.hindiTvShows);
      await tvShowsBox.put('hindiTvShowsTimestamp', DateTime.now().toIso8601String());

      return widget.hindiTvShows;
    } catch (e) {
      return [];
    }
  }
