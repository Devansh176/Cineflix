...
class _HindiWesternSeriesState extends State<HindiWesternSeries> {
  ...
  Future<List> loadSeries() async {
    try {
      if (!isHiveInitialized) {
        await _initializeHiveOnce();
      }

      final cachedData = seriesWesternBox.get('hindiWesternSeries');
      final cachedTimestamp = seriesWesternBox.get('hindiWesternSeriesTimestamp');
      ...
      await seriesWesternBox.put('hindiWesternSeries', widget.hindiWesternSeries);
      await seriesWesternBox.put('hindiWesternSeriesTimestamp', DateTime.now().toIso8601String());

      return widget.hindiWesternSeries;
    } catch (e) {
      return [];
    }
  }
