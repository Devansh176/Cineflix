...
class HindiActionSeries extends StatefulWidget {
  const HindiActionSeries({super.key, required this.hindiActionSeries});
  final List hindiActionSeries;

  @override
  State<HindiActionSeries> createState() => _HindiActionSeriesState();
}

class _HindiActionSeriesState extends State<HindiActionSeries> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('ActionSeriesBox')) {
          seriesActionBox = Hive.box('ActionSeriesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesActionBox.get('hindiActionSeries');
      final cachedTimestamp = seriesActionBox.get('hindiActionSeriesTimestamp');
      ...
      await seriesActionBox.put('hindiActionSeries', widget.hindiActionSeries);
      await seriesActionBox.put('hindiActionSeriesTimestamp', DateTime.now().toIso8601String());

      return widget.hindiActionSeries;
    } catch (e) {
      return [];
    }
  }
