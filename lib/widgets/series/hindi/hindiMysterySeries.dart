...
class HindiMysterySeries extends StatefulWidget {
  const HindiMysterySeries({super.key, required this.hindiMysterySeries});
  final List hindiMysterySeries;

  @override
  State<HindiMysterySeries> createState() => _HindiMysterySeriesState();
}

class _HindiMysterySeriesState extends State<HindiMysterySeries> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('seriesMysteryBox')) {
          seriesMysteryBox = Hive.box('seriesMysteryBox');
        } else {
          seriesMysteryBox = await Hive.openBox('seriesMysteryBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesMysteryBox.get('hindiMysterySeries');
      final cachedTimestamp = seriesMysteryBox.get('hindiMysterySeriesTimestamp');
      ...
      await seriesMysteryBox.put('hindiMysterySeries', widget.hindiMysterySeries);
      await seriesMysteryBox.put('hindiMysterySeriesTimestamp', DateTime.now().toIso8601String());

      return widget.hindiMysterySeries;
    } catch (e) {
      return [];
    }
  }
