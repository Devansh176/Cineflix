...
class HindiDocumentarySeries extends StatefulWidget {
  const HindiDocumentarySeries({super.key, required this.hindiDocumentarySeries});
  final List hindiDocumentarySeries;

  @override
  State<HindiDocumentarySeries> createState() => _HindiDocumentarySeriesState();
}

class _HindiDocumentarySeriesState extends State<HindiDocumentarySeries> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('DocumentarySeriesBox')) {
          seriesDocumentaryBox = Hive.box('DocumentarySeriesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = seriesDocumentaryBox.get('hindiDocumentarySeriesTimestamp');
      ...
      await seriesDocumentaryBox.put('hindiDocumentarySeries', widget.hindiDocumentarySeries);
      await seriesDocumentaryBox.put('hindiDocumentarySeriesTimestamp', DateTime.now().toIso8601String());

      return widget.hindiDocumentarySeries;
    } catch (e) {
      return [];
    }
  }
