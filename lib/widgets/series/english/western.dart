...
class WesternSeries extends StatefulWidget {
  const WesternSeries({super.key, required this.westernSeries});
  final List westernSeries;

  @override
  State<WesternSeries> createState() => _WesternSeriesState();
}

class _WesternSeriesState extends State<WesternSeries> {
  late Box seriesWesternBox;
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('WesternSeriesBox')) {
          seriesWesternBox = Hive.box('WesternSeriesBox');
        } else {
          seriesWesternBox = await Hive.openBox('WesternSeriesBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesWesternBox.get('westernSeries');
      final cachedTimestamp = seriesWesternBox.get('westernSeriesTimestamp');
      ...
      await seriesWesternBox.put('westernSeries', widget.westernSeries);
      await seriesWesternBox.put('westernSeriesTimestamp', DateTime.now().toIso8601String());

      return widget.westernSeries;
    } catch (e) {
      return [];
    }
  }
