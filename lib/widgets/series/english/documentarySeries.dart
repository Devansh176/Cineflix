...
class DocumentarySeries extends StatefulWidget {
  const DocumentarySeries({super.key, required this.documentarySeries});
  final List documentarySeries;

  @override
  State<DocumentarySeries> createState() => _DocumentarySeriesState();
}

class _DocumentarySeriesState extends State<DocumentarySeries> {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesDocumentaryBox.get('documentarySeries');
      final cachedTimestamp = seriesDocumentaryBox.get('documentarySeriesTimestamp');
      ...
      await seriesDocumentaryBox.put('documentarySeries', widget.documentarySeries);
      await seriesDocumentaryBox.put('documentarySeriesTimestamp', DateTime.now().toIso8601String());

      return widget.documentarySeries;
    } catch (e) {
      return [];
    }
  }
