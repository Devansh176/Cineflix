...
class DramaSeries extends StatefulWidget {
  const DramaSeries({super.key, required this.dramaSeries});
  final List dramaSeries;

  @override
  State<DramaSeries> createState() => _DramaSeriesState();
}

class _DramaSeriesState extends State<DramaSeries> {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesDramaBox.get('dramaSeries');
      final cachedTimestamp = seriesDramaBox.get('dramaSeriesTimestamp');
      ...
      await seriesDramaBox.put('dramaSeries', widget.dramaSeries);
      await seriesDramaBox.put('dramaSeriesTimestamp', DateTime.now().toIso8601String());

      return widget.dramaSeries;
    } catch (e) {
      return [];
    }
  }
