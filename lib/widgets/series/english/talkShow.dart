...
class TalkShow extends StatefulWidget {
  const TalkShow({super.key, required this.talkShow});
  final List talkShow;

  @override
  State<TalkShow> createState() => _TalkShowState();
}
...
class _TalkShowState extends State<TalkShow> {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = talkShowBox.get('talkShow');
      final cachedTimestamp = talkShowBox.get('talkShowTimestamp');
      ...
      await talkShowBox.put('talkShow', widget.talkShow);
      await talkShowBox.put('talkShowTimestamp', DateTime.now().toIso8601String());

      return widget.talkShow;
    } catch (e) {
      return [];
    }
  }
