...
class ComedySeries extends StatefulWidget {
  const ComedySeries({super.key, required this.comedySeries});
  final List comedySeries;

  @override
  State<ComedySeries> createState() => _ComedySeriesState();
}

class _ComedySeriesState extends State<ComedySeries> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('ComedySeriesBox')) {
          seriesComedyBox = Hive.box('ComedySeriesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = seriesComedyBox.get('comedySeriesTimestamp');
      ...
      await seriesComedyBox.put('comedySeries', widget.comedySeries);
      await seriesComedyBox.put('comedySeriesTimestamp', DateTime.now().toIso8601String());

      return widget.comedySeries;
    } catch (e) {
      return [];
    }
  }
