...
class RealityShow extends StatefulWidget {
  const RealityShow({super.key, required this.realityShow});
  final List realityShow;

  @override
  State<RealityShow> createState() => _RealityShowState();
}

class _RealityShowState extends State<RealityShow> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('seriesRealityBox')) {
          seriesRealityBox = Hive.box('seriesRealityBox');
        } else {
          seriesRealityBox = await Hive.openBox('seriesRealityBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = seriesRealityBox.get('realityShow');
      final cachedTimestamp = seriesRealityBox.get('realityShowTimestamp');
      ...
      await seriesRealityBox.put('realityShow', widget.realityShow);
      await seriesRealityBox.put('realityShowTimestamp', DateTime.now().toIso8601String());

      return widget.realityShow;
    } catch (e) {
      return [];
    }
  }
