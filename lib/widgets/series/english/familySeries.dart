...
class FamilySeries extends StatefulWidget {
  const FamilySeries({super.key, required this.familySeries});
  final List familySeries;

  @override
  State<FamilySeries> createState() => _FamilySeriesState();
}

class _FamilySeriesState extends State<FamilySeries> {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = seriesFamilyBox.get('familySeriesTimestamp');
      ...
      await seriesFamilyBox.put('familySeries', widget.familySeries);
      await seriesFamilyBox.put('familySeriesTimestamp', DateTime.now().toIso8601String());

      return widget.familySeries;
    } catch (e) {
      return [];
    }
  }
