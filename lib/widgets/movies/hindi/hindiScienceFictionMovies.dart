...
class HindiScienceFictionMovies extends StatefulWidget {
  const HindiScienceFictionMovies({super.key, required this.scienceFictionHindiMovies});
  final List scienceFictionHindiMovies;
...
class _HindiScienceFictionMoviesState extends State<HindiScienceFictionMovies> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('ScienceFictionMoviesBox')) {
          scienceFictionBox = Hive.box('ScienceFictionMoviesBox');
        } else {
          scienceFictionBox = await Hive.openBox('ScienceFictionMoviesBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = scienceFictionBox.get('sciFiHindiMovies');
      final cachedTimestamp = scienceFictionBox.get('sciFiHindiMoviesTimestamp');
      ...
      await scienceFictionBox.put('sciFiHindiMovies', widget.scienceFictionHindiMovies);
      await scienceFictionBox.put('sciFiHindiMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.scienceFictionHindiMovies;
    } catch (e) {
      return [];
    }
  }
