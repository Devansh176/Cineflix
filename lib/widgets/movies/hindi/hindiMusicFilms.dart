...
class HindiMusicFilms extends StatefulWidget {
  const HindiMusicFilms({super.key, required this.musicHindiFilms});
  final List musicHindiFilms;

  @override
  State<HindiMusicFilms> createState() => _HindiMusicFilmsState();
}

class _HindiMusicFilmsState extends State<HindiMusicFilms> {
  late Box musicBox;
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('MusicFilmsBox')) {
          musicBox = Hive.box('MusicFilmsBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = musicBox.get('musicHindiFilms');
      final cachedTimestamp = musicBox.get('musicHindiFilmsTimestamp');
      ...
      await musicBox.put('musicHindiFilms', widget.musicHindiFilms);
      await musicBox.put('musicHindiFilmsTimestamp', DateTime.now().toIso8601String());

      return widget.musicHindiFilms;
    } catch (e) {
      return [];
    }
  }
