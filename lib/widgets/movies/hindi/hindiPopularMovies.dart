...
class HindiPopularMovies extends StatefulWidget {
  const HindiPopularMovies({super.key, required this.popularHindiMovies});
  final List popularHindiMovies;
...
class _HindiPopularMoviesState extends State<HindiPopularMovies> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('PopularMoviesBox')) {
          popularBox = Hive.box('PopularMoviesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = popularBox.get('popularHindiMoviesTimestamp');
      ...
      await popularBox.put('popularHindiMovies', widget.popularHindiMovies);
      await popularBox.put('popularHindiMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.popularHindiMovies;
    } catch (e) {
      return [];
    }
  }
