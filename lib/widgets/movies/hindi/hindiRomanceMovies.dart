...
class HindiRomanceMovies extends StatefulWidget {
  const HindiRomanceMovies({super.key, required this.romanceHindiMovies});
  final List romanceHindiMovies;
...
class _HindiRomanceMoviesState extends State<HindiRomanceMovies> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('RomanceMoviesBox')) {
          romanceBox = Hive.box('RomanceMoviesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = romanceBox.get('romanceHindiMoviesTimestamp');
      ...
      await romanceBox.put('romanceHindiMovies', widget.romanceHindiMovies);
      await romanceBox.put('romanceHindiMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.romanceHindiMovies;
    } catch (e) {
      return [];
    }
  }
