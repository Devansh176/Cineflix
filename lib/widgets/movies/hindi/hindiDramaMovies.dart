...
class HindiDramaMovies extends StatefulWidget {
  const HindiDramaMovies({super.key, required this.dramaHindiMovies});
  final List dramaHindiMovies;
...
class _HindiDramaMoviesState extends State<HindiDramaMovies> {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedTimestamp = dramaBox.get('dramaHindiMoviesTimestamp');
      ...
      await dramaBox.put('dramaHindiMovies', widget.dramaHindiMovies);
      await dramaBox.put('dramaHindiMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.dramaHindiMovies;
    } catch (e) {
      return [];
    }
  }
