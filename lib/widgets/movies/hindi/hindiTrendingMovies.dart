...
class HindiTrendingMovies extends StatefulWidget {
  const HindiTrendingMovies({super.key, required this.trendingHindiMovies});
  final List trendingHindiMovies;
...
class _HindiTrendingMoviesState extends State<HindiTrendingMovies> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('TrendingMoviesBox')) {
          trendingBox = Hive.box('TrendingMoviesBox');
        } else {
          trendingBox = await Hive.openBox('TrendingMoviesBox');
        }
        setState(() {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = trendingBox.get('trendingHindiMovies');
      final cachedTimestamp = trendingBox.get('trendingHindiMoviesTimestamp');
      ...
      await trendingBox.put('trendingHindiMovies', widget.trendingHindiMovies);
      await trendingBox.put('trendingHindiMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.trendingHindiMovies;
    } catch (e) {
      return [];
    }
  }
