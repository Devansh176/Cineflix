...
class MysteryMovies extends StatefulWidget {
  const MysteryMovies({super.key, required this.mysteryMovies});
  final List mysteryMovies;

  @override
  State<MysteryMovies> createState() => _MysteryMoviesState();
}

class _MysteryMoviesState extends State<MysteryMovies> {
  late Box mysteryBox;
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('MysteryMoviesBox')) {
          mysteryBox = Hive.box('MysteryMoviesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = mysteryBox.get('mysteryMovies');
      final cachedTimestamp = mysteryBox.get('mysteryMoviesTimestamp');
      ...
      await mysteryBox.put('mysteryMovies', widget.mysteryMovies);
      await mysteryBox.put('mysteryMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.mysteryMovies;
    } catch (e) {
      return [];
    }
  }
