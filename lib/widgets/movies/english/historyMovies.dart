...
class HistoryMovies extends StatefulWidget {
  ...
  @override
  State<HistoryMovies> createState() => _HistoryMoviesState();
}

class _HistoryMoviesState extends State<HistoryMovies> {
  late Box historyBox;
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('HistoryMoviesBox')) {
          historyBox = Hive.box('HistoryMoviesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = historyBox.get('historyMovies');
      final cachedTimestamp = historyBox.get('historyMoviesTimestamp');
      ...
      await historyBox.put('historyMovies', widget.historyMovies);
      await historyBox.put('historyMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.historyMovies;
    } catch (e) {
      return [];
    }
  }
