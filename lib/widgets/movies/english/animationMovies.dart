...
class AnimationMovies extends StatefulWidget {
  ...
  @override
  State<AnimationMovies> createState() => _AnimationMoviesState();
}

class _AnimationMoviesState extends State<AnimationMovies> {
  ...
  Future<void> _initializeHiveOnce() async {
    if (!isHiveInitialized) {
      try {
        if (Hive.isBoxOpen('AnimationMoviesBox')) {
          animationBox = Hive.box('AnimationMoviesBox');
        } else {
  ...
  Future<List> loadMovies() async {
    try {
      ...
      final cachedData = animationBox.get('animationMovies');
      final cachedTimestamp = animationBox.get('animationMoviesTimestamp');
      ...
      await animationBox.put('animationMovies', widget.animationMovies);
      await animationBox.put('animationMoviesTimestamp', DateTime.now().toIso8601String());

      return widget.animationMovies;
    } catch (e) {
      return [];
    }
  }
